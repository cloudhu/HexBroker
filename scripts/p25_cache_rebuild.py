#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""P25-2 v13 信号缓存重建编排：v8 同口径 → v13（每组合 checkpoint 落盘 + resume）。

背景
----
P25-2 需以当前数据（K 线到 2026-08-21）完整重跑 walk_forward，口径与 v8
完全一致（仅数据范围相同——当前数据即 P21 重建时数据，无新增）：
  ...
  - 输出 artifacts/signals_cache18_grouped_v13.parquet（不覆盖 v8）

P13 教训：后台任务可能被回收 → 按组（8 组）独立落盘 checkpoint
（artifacts/p25_checkpoints/{group}.parquet），resume 时跳过已有组，
最后合并 8 个 checkpoint → v13。结果与一次性全量运行逐字节一致
（各组独立训练，合并仅 concat，无跨组状态）。

P25-0 前置探针（scripts/p25_0_fold_probe.py）已证实：当前数据 fold 网格
未跨边界（第 31 折 test_end=2117 > n=2093，还需 20-24 个交易日）→ 预期
v13 与 v8 逐字节一致；本脚本产出实证 artifact 供 byte-compare 确认。

用法
----
  python scripts/p25_cache_rebuild.py                     # 全量（跳过已有 checkpoint）
  python scripts/p25_cache_rebuild.py --only agri_protein # 只跑单组（冒烟/续跑）
  python scripts/p25_cache_rebuild.py --force             # 删除已有 checkpoint 重跑
  python scripts/p25_cache_rebuild.py --skip-rebuild      # 仅合并已有 checkpoint → v13
"""
...
# v8 各组合计行数（artifacts/p8_4_rebuild_v8.log 记录），用于校验 v13 组行数一致
V8_GROUP_ROWS = {
    "precious": 960,
    "ferrous_steel": 960,
    "ferrous_raw": 1440,
...
def merge_checkpoints() -> Path:
    """合并 8 组 checkpoint → v13；按 v8 组行数校验。"""
    CKPT_DIR.mkdir(parents=True, exist_ok=True)
    ...
    for group in GROUPS_V2:
        ...
        if actual != expected:
            problems.append(f"{group}: v13 行数 {actual} != v8 行数 {expected}")
    if not frames:
        raise SystemExit("[FAIL] 无 checkpoint 可合并")
    all_sig = pd.concat(frames, ignore_index=True)
    ART.mkdir(exist_ok=True)
    all_sig.to_parquet(V13_PATH, index=False)
    print(f"[OK] v13 合并 {len(all_sig)} 条 → {V13_PATH}")
    if problems:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="P25-2 v13 信号缓存重建（checkpoint+resume）")
    ...
    ap.add_argument("--skip-rebuild", action="store_true", help="仅合并已有 checkpoint → v13")
    args = ap.parse_args()

    print("=" * 72)
    print("P25-2 v13 信号缓存重建（v8 同口径：cross_z + label_pool=all + cal_split=0.5）")
    print("=" * 72)
