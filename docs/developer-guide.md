...
**P20-4（进行中）**：基差增量补齐（18 品种到 08-20）+ p16 完整性检查修复（basis_latest < target_date 即 WARN，消除"PASS 与无信号并存"）+ 空计划日语义标注（区分数据不完整 vs 真实空仓）。

---
*文档版本：v3.24（2026-08-21，Sentinel-2 P20 实盘运行）｜ 关联报告：deliverables/software-hexfutures-ai/（40+ 份实验报告）*
