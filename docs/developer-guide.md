...
**数据刷新**（08-18~08-20 增量，QA VERIFIED）：18 品种 K 线全到 08-20（RAW_SCALE_FIX 换算逐位正确、边界无伪跳变）；Sentinel-2 进入实盘运行阶段。
...
- **数据刷新**：18 品种 K 线全到 **2026-08-21**（RAW_SCALE_FIX 换算偏差 0%）；基差 08-21 **源端未发布**（17 品种保持 08-20、SC 04-30）→ 08-21 计划=空计划（plan_note "数据不完整"标注，P22 待补拉重出）
