...
| 观察项2 | 脏行自证循环（缓存残留一行当日信号即可自证 fd=0 过闸） | 认同「原理上门禁用同一份缓存既做交易日证据又做被判对象，无法自检」 | **已落缓解工具**：`p6_5_cache_postfill_guard.py`（三项检查全部用独立证据源，见 §九 #4） |
...
### 9.1 新增工具：`scripts/p6_5_cache_postfill_guard.py`（QA 观察项 2 缓解，本报告入库后追加）

与 `p6_4_backfill_guard`（管「缓存**旧不旧**」）成对：**管「缓存内容干净不干净」**。三项检查，全部使用**独立证据源**（自证循环破局点）：

| 检查 | 独立证据 | 结果 |
|---|---|---|
| 脏信号日 | 主湖并集日历 + 节假日表（**绝不**喂缓存自己的信号日） | 真实生产缓存独立复现 QA 报的 4 个脏日期，且**细化到品种**：9 行脏数据（m0@2020-10-02；ag0/au0/m0@2021-10-01；同 2022-04-04；ag0/au0@2024-06-10），两个缓存各 9 行 |
| 当日行完整性 | `paper.symbols` 配置权威清单（3 个**交易品种**，非缓存 18 研究品种去重） | 2026-08-31 夜盘视角：ag0/c0/rb0 全缺当日行 = 今天尚未刷新（真实状态，非漏检） |
| 内容卫生 | `p_up ∈ [0,1]`、NaN、ts 可解析 | 当前干净 |

- ⛔ 只判定、不删改：脏行剔除是策略决策（先例：P2-B 幽灵行由主理人拍板），工具只报告。
- 用法：`python scripts/p6_5_cache_postfill_guard.py --session night`（20:30 补数后）；
  退出码 0/2/3/10/11 与 p6_4 契约同构，可直接接自动化。
- 测试：`tests/test_p6_5_cache_postfill_guard.py` 11 项（含**测试先行揪出的真 bug**：
  初版缺 `ref_day` 截断，未来日期行会被「> cal_max 且是交易日」分支洗白 —— 与
  `augment_calendar` 截断层语义不一致，已修并钉死）。

### 9.2 挂账清单

| # | 项 | 优先级 |
|---|---|---|
| 1 | 今晚 20:30 夜盘观察：确认 20:55 引擎启动后日盘可正常主源开仓；**补数后跑 `p6_5 --session night`** | P0 |
| 2 | QA 环境恢复后补独立实跑复核（Read `_qa_verify_stdout_20260831.txt` 逐项对 + 自跑一遍） | P0 |
| 3 | 主源 v8 缓存 `fd=41` 陈旧（靠 tail_ext 取 min 达标）—— 单独立项 | P1 |
| 4 | 9 行脏数据的**剔除裁决**（品种级明细见 9.1，建议随主源 v8 缓存 fd=41 立项一并处置） | P1 |
