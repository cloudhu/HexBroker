...
> 第五轮修复后：全量 pytest **305/305 通过**（62s），较第四轮 298 净增 **7** 个回归测试（L3×2/L4×1/L5×1/V1×2/V5×5 共 11 个测试函数落地，其中 test_feature_normalize.py 为既有文件改写、其余 4 个为新增测试文件，净增 7 用例）。Batch B 5 项均经 fresh-eyes 源码复核确认 + 运行时/机制验证 + 回归测试，未触及研究结论（生产口径 `CONTRACTS18`/`group_cap=0.5` 不变）。
...
> 勘误：本报告各轮全量数字以最终实测 315/315 为准，个别轮次表述已按 QA fresh-eyes 复核校正。二-E 原表述「新增 11」含既有测试文件改写，净增为 7 用例（二-E 现按 305/305 口径）。
