...
| 项 | 结果 |
|---|---|
| 测试 | **853 passed**（677 → … → 836 → 841 → 843 → 853，+5 包络修复，+2 manifest 回填，+10 名义价回填） |
| 静态门禁 | 改动文件 ruff **全通过** |
...
4. **下一步优先级**：P0-9 ✅、编排器 ✅、P0-10 审计 ✅（处置待拍板）、P0-12 ✅、P0-13 ✅、P1-b ✅、P1-c ✅（存量回填待拍板）。剩余拍板项 = cu0/rb0 2023 处置 / raw_close 存量回填；剩余任务 = P0-11（卡授权）、扁平层 manifest 另案。
