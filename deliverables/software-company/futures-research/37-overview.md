...
| **P0-10** | 年度口径审计（18 品种 × 9 年）：**确认仅 cu0/rb0 的 2023 名义价冒充**（±27%~51% 假跳空）；检测器固化 `caliber`；**处置完成（§4.20）**：主理人拍板 ③真值重建 —— p11 驱动器泛化为整年替换（前置覆盖预检：真值日期必须全覆盖分区，脏行不得静默残留）→ RB/CU 2023 各 242 行替换 → 假跳变消失（1.02%/1.12%/0.72%/0.06%）+ caliber 复扫 0 告警（定罪检测器判无罪，证据闭环） | ✅ |
| **P0-12** | 缺失年度显式化：`load_processed` 洞/标记逐条告警（数据行为不变）+ `quality_notes` 三类结构化提示 | ✅ |
| **P1-c** | 🆕 `raw_close` 列语义修复：parse 阶段 `enrich_raw_close` 备源名义价回填（失败大声降级），实测 raw_close ≠ adj_close；**存量回填完成（§4.21）**：主理人拍板 ①离线批量 —— p37 驱动器（每品种预热 + 幂等跳过 + apply 前全量备份 + manifest 双语义）；首轮 dry-run 162/162 BackupExhaustedError 根因定罪（未来窗口 end=2099 被 assert_fresh 判陈旧，豁免条件 end < today-5d 不覆盖）→ warm 按品种实际范围 + end 钳制今天修复；最终 **156 分区 / 32,220 行回填**、备份可回滚、幂等复跑 0、零污染 QA、869 passed | ✅ |
...
| **P2-rawclose** | 🆕 **raw_close 单日口径残留全湖定罪 + 修复闭环（§4.24/§4.25）**：p42 对账探针副产发现（161 事件跨品种同日聚集：2019-04-22 ×12 / 2021-12-30 ×11）；p43 修复驱动器（数学精确 `raw_true = adj/k` + G1/G2/G3 三重安全门 + dry-run 默认 + 内置复扫门禁 + sidecar 留痕）；akshare 独立验证 cu0 2019-04-22 源端整行缺失；**--apply 已执行（2026-08-29 主理人拍板）**：preflight 与存档计划逐条一致 → 82 分区 / 161 行重写 → 复扫 0 事件 → 幂等复跑 0 → sidecar 单条留痕；901 passed | ✅ |
