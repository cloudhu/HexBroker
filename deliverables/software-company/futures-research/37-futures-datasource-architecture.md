...
- [ ] **P0-6 换月检测与告警**：转向**事后对账**路线 —— 主源恢复后重算重叠区比值、
      检出跳变并触发窗口重建（不再追求"前向预测换月"，实测不可靠）
...
| `hexbroker/data/test_graft.py` | 🆕 22 测试（含真实事故模式固化用例） |
| `scripts/dev_probe_37_graft_truth.py` | 🆕 真实数据端到端反证 + 三策略消融（§4.8） |

**验证**：**752 passed**（原 677 → 713 → 730 → 751 → 752）；改动文件 ruff 全通过；
真实联网 18/18 双源末日 2026-08-28；`git fsck --no-dangling` 无输出；`data/` 零改动。
