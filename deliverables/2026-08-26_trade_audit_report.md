...
## 修订记录（2026-08-26 续，P1-1 近实盘取证后回灌）

> 背景：用户指令"在沙箱拉起当前源码取证 P1-1"。本地无运行实例（日盘 15:00 已收），用 `scripts/p1_1_live_evidence.py` 做**近实盘取证**：真实 `TradingScheduler`/`RiskGate`/`SignalEngine`/实时新浪行情，状态重定向到临时目录（不碰实盘快照/pid 锁）。信号评估源最多到 8-25，本进程临时放宽 `SignalEngine._freshness_threshold=1` 使 `is_effective=True` 进入门禁/冷却真实分支。

**实证日志行（原文，已落盘 `deliverables/2026-08-26_p1_1_deploy_evidence.md`）**：
| 行 | 内容 |
|---|---|
| 冷却拦截（要求 A） | `[PAPER] 品种 ag0 信号未变化（p_up=0.8000 exp_ret=0.9238 src=engine_a），冷却拦截重复开仓`（rb0 同款，各 2 条） |
| 成本门禁（要求 B，**非 0 算值**） | `[PAPER] 成本门禁拦截开仓 symbol=rb0 … expected_pnl=123.61 round_trip_cost=24.61 notional=30710.00`（旧 bug 是 0.00/0.00） |
| 门禁算值复核 | `ag0 exp_pnl=2299.98 rt_cost=37.65` / `rb0 exp_pnl=123.61 rt_cost=24.61`（非0判定=True） |

**结论**：P1-1 旧缺陷（8-24 无冷却/门禁行、8-25 `expected_pnl=0.00` 误拦+刷屏 128 次）**当前源码已根治**。两类要求行均真实打印，公式算值正确，去重指纹跨 tick 持久化（P2-4）。全量 `tests/` **429/429 passed**。

- 唯一差异：本取证为沙箱近实盘（非 21:00 夜盘实跑），代码路径一致；**建议夜盘实跑后归档真实 `trades.log` 留底**（rb0 有夜盘）。
- 信号评估源刷新到 8-26 后，`signal_fresh_days: 0`（隔夜过期）即让当日信号 `is_effective=True` 走真实门禁，无需临时放宽。
...
> 状态：🟢 **P0×0**（P0-1/P0-2/P0-3 全部 ✅已修复/验证）/ 🟢 **P1×3 全部 ✅**（P1-2 ✅已修复、P1-3 ✅已并入 P0-2 HALT、P1-1 ✅近实盘验证 / 仅余夜盘留底归档）/ 🟢 **P2×0 待修**（P2-2/P2-3/P2-4/P2-5 ✅已修复/验证；**P2-1 ⛔外部数据阻塞**，非可修缺陷）。
