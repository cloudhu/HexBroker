...
### 46. P1 组内细分（2026-08-18 下午，负面结果+根因修正）
- 报告：`deliverables/software-hexfutures-ai/group-subdivide-2026-08-18.md`
- v2 细分与 v1 完全等价（IC Δ=0、净值一致）——walk_forward 逐品种独立训练，分组仅影响特征；
- **p0 根因修正**：非组内压制，是自身信号负 IC（缺产业特征）——修复需产业数据或剔除；
- **当前最优确认**：v1 分组+0.8/0.2/top35%/W20-linear → 全样本年化 17.12%/Sharpe 1.00；OOS 0.33。
...
### 47. P0 p0 剔除实验（2026-08-18 下午，OOS 0.33→0.40）
- 报告：`deliverables/software-hexfutures-ai/p0-removal-2026-08-18.md`
- **剔 p0 净正**：全样本 1.00→0.99（微降）、**OOS 0.33→0.40**（+21%）、回撤 -3.4%；
- 批量剔负 IC 反伤全样本（0.85）——负 IC 品种 OOS 有分散化价值，只剔最差；
- **当前最优 v1.1**：v1 分组 17 品种 + 0.8/0.2/top35%/W20-linear → 全样本 16.92%/Sharpe 0.99；OOS 0.40。
...
### 49. P2 回撤控制（2026-08-18 下午，部署级配置确立） 
- 报告：`deliverables/software-hexfutures-ai/drawdown-control-2026-08-18.md`
- **波动率目标 17.5% 采纳**：回撤 -26.1%→**-18.25%**（-30%）、Sharpe 0.98→0.94（仅 -4%）、年化 14.31%；网格平滑非尖峰；
- 止损否决（几乎不触发）、熔断否决（误伤反弹，Sharpe 0.61-0.77）；
- **部署级配置 v2.0**：17 品种分组 + 0.8exp/0.2mom90 + top35% + W20-linear + 波目标17.5% → 全样本 Sharpe 0.94/回撤 -18.25%；OOS 0.24。
